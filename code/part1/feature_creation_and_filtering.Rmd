...
power_features_sentence = function(corpus) {
  ...
  for (i in 1:n) {
    ...
    sents2 = unlist(strsplit(sents, split = ".", fixed = T))
    if (x==1) {
...
```{r}
# Import & Clean
htest <- read.table('../../data/test/HRC_test.tsv', header = TRUE, stringsAsFactors = FALSE)[,1]
test_power_feat <- Corpus(VectorSource(clean_pw(htest)))
hrc_test <- Corpus(VectorSource(clean(htest)))
...
final_features_test <- cbind(test_pwf_sent, test_pwf_dtm, test_dtm)
