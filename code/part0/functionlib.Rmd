...
```{r}
# 1.
import_text_documents = function(directory = getwd(), doc_dirs = list(no_type=""), file_pattern = "\\.tsv$") {

  file_names = list.files(path=directory, pattern=file_pattern, full.names=F, recursive=T)
  cat(paste0("Found ", length(file_names), " .tsv files in \"", directory, "\" to import.\n"))
...
### external function(2): removes punctuations, numbers, single/two-lettered words and multiple whitespaces.
remove_extra = function(x) {
  ...
  x <- trimws(gsub("\\s+", " ", x)) # Removes multiple whitespaces.
  return(x)
}
...
clean_documents = function(hrc, stopwords = c()) {
  ...
  # Turn VCorpus into a DocumentTermMatrix.
  dtm = DocumentTermMatrix(hrc, control = list(stemming=T))
  
  # Removing terms that occurs less than 1% of the time in the document.
...
power_features_sentence = function(doc) {
  ...
  for (i in 1:n) {
    ...
    power7 = sum(stri_count(sents, regex="\\S+")) / max(1, length(sents))
    
    ### We need to re-create power feature from this point on because we deleted all numbers and puntuations. (so that is 4 more power features according to this chunk.)###
    # Number of 4-digit numbers (years). 
    #power8 = length(na.omit(str_extract(sents, "\\d{4}")))
    
    # Number of digits.
    # CK: actually, this is number of sentences that contain a digit.
    #power9 = sum(grepl("[[:digit:]]", sents))
