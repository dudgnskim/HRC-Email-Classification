...
### external function(2): removes punctuations, numbers, single/two-lettered words, repeating letters and multiple whitespaces.
remove_extra = function(x) {
  # Removing multiple punctuations, numbers, single/two-lettered words, repeating letters (more than 3 times) and whitespaces. (In that order)
  gsub("\\s+", " ", 
       gsub("\\b(\\S+?)\\1\\S*\\b", "",
            gsub("*\\b[[:alpha:]]{1,2}\\b *", " ",
                 gsub("[[:digit:]]", "", gsub("[[:punct:]]", "", x))), perl = TRUE))
}
