...
# hrc_tr <- docs[[1]][[2]]$content
# This yields a data.frame with TWO columns (V1: response variable (categorical response values), V2: email content (soon-to-be predictor space)).
...
#OR I think we could use this too.
...
load_stopwords = function(input_file = '../../data/common-english-words.txt',
  ...
  if (reload_file && length(output_file) > 0 && file.exists(output_file)) {
    ...
    com_sw <- as.character(read.table('../../data/common-english-words.txt', sep = ',',
...
# Load stopwords
load_stopwords() # I tried to find a way to actually load the stopwords inside the function, but it doesn't seem so easy..
