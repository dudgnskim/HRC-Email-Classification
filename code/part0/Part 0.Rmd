...
import_text_documents = function(directory = "", doc_dirs = list(no_type=""), file_pattern = "\\.tsv$") {
  ...
  file_names = list.files(path=directory, pattern="*\\.tsv", full.names=F, recursive=T)
  cat(paste0("Found ", length(file_names), " text files in \"", directory, "\" to import.\n"))

  if (length(file_names) == 0) {
    stop(paste("did not find any files to load. Make sure you have unzipped the training",
...
docs <- import_text_documents(getwd()) #Set the directory of the function to current directory by using getwd() which loads the .tsv file in the directory
```
...
# For step 1.
load_stopwords = function(input_file = file.choose(), output_file = "stopwords.Rda",reload_file=F) {
...
#file.choose() allow manually select the common-english-words.text file 

load("stopwords.Rda") #load the variable "stopwords" into the global environment
```

###Clean documents
```{r}
# Derive a dictionary of words/ bigrams and total number of their appearances through out the whole dataset.
clean_documents = function(hrc, stopwords = stopwords) {
  hrc = tm_map(hrc, content_transformer(tolower))

  # hrc = tm_map(hrc, removeWords)
...
hrc <- read.table(file.choose(), stringsAsFactors = FALSE) #load the HRC_train.tsv file
Unique_words <- clean_documents(Corpus(VectorSource(hrc$V2)), stopwords) #gives unique words after removing stopwords
```

###Clean imported documents
```{r}
# For step 2; docs is a list of matrices loaded in step 1.
clean_imported_documents = function(docs = docs, stopwords = stopwords) {
  ...
  #foreach (type in names(docs)) %dopar% {
    ...
    #cleaned_docs[[type]] = clean_documents(docs[[type]], stopwords)
      doc_set = clean_documents(Corpus(VectorSource(docs)), stopwords)

      # Fix rownames - remove the .tsv suffix from the name.
...
dummy <- clean_imported_documents()
```
...
###Power feature sentences
```{r}
# For step 1.
doc_6 <- Corpus(VectorSource(hrc$V2))
power_features_sentence = function(doc = doc_6) {
  n = length(doc)
...
power_feat_sentences <- power_features_sentence() #outcome a matrix that uses sentence as power features
```
...
pwr_feat_dtm <- power_features_dtm(Unique_words) #returns the power feature using words in the emails
```
...
NgramTokenizer = function(x, ngram = 2) {
  # Specify NLP package because qdap also provides the ngrams function.
  unlist(lapply(NLP::ngrams(words(x), ngram), paste, collapse = " "), use.names = FALSE)
}

BigramTokenizer = function(x, ngram = 2) {
  NgramTokenizer(x, ngram)
}

# Separate function because it's unclear how to change a parameter in the DocumentTermMatrix call.
TrigramTokenizer = function(x, ngram = 3) {
  NgramTokenizer(x, ngram)
}
...
hrc_8 = Corpus(VectorSource(hrc$V2))

power_features_ngrams = function(hrc = hrc_8, stopwordss = stopwords, ngram = 2, min_sparsity = 0.99) {
  hrc = tm_map(hrc, content_transformer(tolower))
  hrc = tm_map(hrc, removePunctuation)
  hrc = tm_map(hrc, removeNumbers)

  if (length(stopwords) > 0) {
    hrc  = tm_map(hrc, removeWords, stopwordss)
  }
  ...
  dtm_ngrams = DocumentTermMatrix(hrc,
                           control = list(tokenize = f, stopwordss=T, stemming=T))
...
pwr_feat_ngrams <- power_features_ngrams()
```
